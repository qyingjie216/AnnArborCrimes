...
## Is Ann Arbor a relatively more dangerous city in Michigan?

We care more about the big cities in Michigan, so the 20 cities with highest population are selected for comparison.
```{r}
df2019 %>%
...
As the plot shown above, Detroit is the only outlier. We would like to explore in another way that if Ann Arbor has a relatively higher crime rate.
...
Property crime in Ann Arbor is much more common than violent crime. In 2019, the FBI reported a total of 2124 property crimes, compared with 309 violent crimes.
...
```{r}
plot_grid(p_may,p_sep,p_oct,ncol=1,heights=c(10,10,10))
```

According to time series plot in May, 05/01, 05/07, 05/10, 05/13, 05/19, 05/22, 05/25 have a relatively high occurrence of cases.

According to time series plot in September, we can see the cases is relatively high on 09/04, 09/11, 09/17, 09/24, and 09/29, which reach the highest on 09/17.

According to time series plot in October, we can see the cases is extremely high on 10/30, comparing to any other days in that month. 10/06, 10/09, 10/15, 10/22 have a relatively high occurrence of cases.
...
During the past 10 years between 2010 and 2019, Violent crime rate in Ann Arbor has an increasing trend while Property crime rate has a decreasing trend.